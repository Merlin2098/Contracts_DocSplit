...
# ==========================================================
# DETECTAR MÓDULOS DEL PROYECTO
# ==========================================================
def detectar_modulos_proyecto():
    """
    Detecta automáticamente todos los módulos .py del proyecto
    para incluirlos como hidden-imports en PyInstaller.
    
    Returns:
        list: Lista de nombres de módulos en formato 'paquete.modulo'
    """
    print("🔍 Detectando módulos del proyecto...")
    ...
    for carpeta in carpetas_modulos:
        ...
        for py_file in carpeta_path.rglob("*.py"):
            # Ignorar __init__.py y __pycache__
            if py_file.name in ["__init__.py", "__pycache__"]:
                continue
            
            # Construir el nombre del módulo (ej: core.utils.logger)
            ruta_relativa = py_file.relative_to(base_dir)
            nombre_modulo = str(ruta_relativa.with_suffix("")).replace(os.sep, ".")
            
            modulos.append(nombre_modulo)
            print(f"   ✅ {nombre_modulo}")
    
    print(f"\n📦 Total de módulos detectados: {len(modulos)}\n")
    return modulos
...
def construir_comando():
    ...
    comando = [
        ...
        "--paths", os.path.join(base_dir, "core", "utils"),
        "--paths", os.path.join(base_dir, "core", "renovaciones"),
        "--paths", os.path.join(base_dir, "core", "contratos"),
        "--paths", os.path.join(base_dir, "controllers"),
        "--paths", os.path.join(base_dir, "gui"),
        "--paths", os.path.join(base_dir, "gui", "widgets"),
        "--paths", os.path.join(base_dir, "gui", "themes"),

        # Dependencias ocultas necesarias (bibliotecas externas)
        "--hidden-import=PyQt5",
    ...
    # ======================================================
    # 🔍 DETECTAR AUTOMÁTICAMENTE MÓDULOS DEL PROYECTO
    # ======================================================
    print("=" * 60)
    modulos_proyecto = detectar_modulos_proyecto()
    
    # Agregar cada módulo detectado como hidden-import
    ...
    # 3. Crear carpeta logs vacía en el bundle (DENTRO de _internal)
    ...
    print(f"   ✅ Incluido: carpeta logs (dentro de _internal)")
...
def detectar_modulos_proyecto():
    ...
    for carpeta in carpetas_modulos:
        ...
        # Buscar todos los archivos .py recursivamente
        for py_file in carpeta_path.rglob("*.py"):
            # Ignorar __init__.py y archivos de prueba
            if py_file.name in ["__init__.py", "__pycache__"]:
                continue
...
def construir_comando():
    ...
    comando = [
        ...
        # Añadir paths MÚLTIPLES VECES para asegurar que los encuentre
        "--paths", base_dir,
        "--paths", os.path.join(base_dir, "core"),
        "--paths", os.path.join(base_dir, "core", "utils"),
        "--paths", os.path.join(base_dir, "core", "renovaciones"),
    ...
    # ======================================================
    # ⚠️ IMPORTANTE: NO incluimos logs con --add-data
    # La carpeta logs se creará FUERA de _internal después
    # ======================================================
    print(f"   ℹ️  Carpeta 'logs' se creará fuera de _internal")
...
# ==========================================================
# CREAR CARPETA LOGS FUERA DE _INTERNAL
# ==========================================================
def crear_carpeta_logs_externa():
    """
    Crea la carpeta 'logs' al mismo nivel que el ejecutable,
    FUERA de la carpeta _internal.
    """
    print("📁 Creando estructura de logs externa...")
    
    carpeta_exe = os.path.join(DIST_PATH, NOMBRE_EXE.replace(".exe", ""))
    logs_externos = os.path.join(carpeta_exe, "logs")
    
    try:
        os.makedirs(logs_externos, exist_ok=True)
        
        # Crear archivo .keep para mantener la carpeta en Git
        keep_file = os.path.join(logs_externos, ".keep")
        with open(keep_file, 'w', encoding='utf-8') as f:
            f.write("# Esta carpeta almacena los logs de la aplicación\n")
            f.write("# Los archivos de log se crean automáticamente\n")
        
        # Crear README opcional con instrucciones
        readme_file = os.path.join(logs_externos, "README.txt")
        with open(readme_file, 'w', encoding='utf-8') as f:
            f.write("=== CARPETA DE LOGS ===\n\n")
            f.write("Esta carpeta contiene los registros de actividad de la aplicación.\n")
            f.write("Los logs se organizan por mes en subcarpetas.\n\n")
            f.write("Puede eliminar los logs antiguos sin afectar el funcionamiento de la app.\n")
        
        print(f"   ✅ Carpeta logs creada: {logs_externos}")
        print(f"   ✅ Archivos auxiliares: .keep y README.txt")
        
    except Exception as e:
        print(f"   ⚠️  Error al crear carpeta logs: {e}")
...
def generar_exe():
    ...
    if result.returncode == 0:
        ...
        # ===== POST-BUILD: CREAR LOGS EXTERNOS =====
        crear_carpeta_logs_externa()
        print()
        
        print(f"✅ Generación completada correctamente.")
        print(f"📂 Carpeta de salida: {carpeta_exe}")
        print(f"📦 Ejecutable: {os.path.join(carpeta_exe, NOMBRE_EXE)}")
        print(f"📁 Logs externos: {os.path.join(carpeta_exe, 'logs')}")
        print(f"\n💡 Para distribuir: Comprime toda la carpeta '{carpeta_exe}'")
        print(f"   Estructura resultante:")
        print(f"   {NOMBRE_EXE.replace('.exe', '')}/")
        print(f"   ├── {NOMBRE_EXE}")
        print(f"   ├── logs/          ← Carpeta para archivos de log")
        print(f"   └── _internal/     ← Dependencias y recursos")
    else:
        print("❌ Error: PyInstaller no se ejecutó correctamente.")
        print("💡 Revisa los mensajes de error arriba para más detalles.")
    print("=" * 60)
